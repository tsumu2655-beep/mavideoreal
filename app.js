...
function mediaExtension(post) {
  ...
  const mime = post.mimeType || (post.type === "video" ? "video/mp4" : "image/jpeg");
...
async function applyZoom(value) {
  ...
  try {
    const zoom = Math.max(state.zoom.min, Math.min(state.zoom.max, Number(value)));
...
function handlePinchStart(event) {
  if (!state.zoom || event.touches.length !== 2) return;

  event.preventDefault();
  state.pinch = {
    distance: touchDistance(event.touches),
    zoom: state.zoom.current,
  };
}

function handlePinchMove(event) {
  if (!state.zoom || !state.pinch || event.touches.length !== 2) return;

  event.preventDefault();
  const ratio = touchDistance(event.touches) / state.pinch.distance;
  applyZoom(state.pinch.zoom * ratio);
}
...
function toggleRecording() {
  ...
  const recorder = createRecorder(state.stream);
  state.mediaRecorder = recorder;
  recorder.ondataavailable = (event) => {
    if (event.data.size) state.recordedChunks.push(event.data);
  };
  recorder.onstop = () => {
    const blob = new Blob(state.recordedChunks, { type: recorder.mimeType || "video/mp4" });
    state.pendingBlob = blob;
    state.pendingMedia = null;
...
els.cameraFrame.addEventListener("touchstart", handlePinchStart, { passive: false });
els.cameraFrame.addEventListener("touchmove", handlePinchMove, { passive: false });
els.cameraFrame.addEventListener("touchend", handlePinchEnd);
els.cameraFrame.addEventListener("touchcancel", handlePinchEnd);
